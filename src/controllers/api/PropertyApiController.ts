...
export class PropertyApiController extends Controller {
    ...
    public onRegister(): void {
        ...
        this.onGet("/api/v1/:lang/property-area/:propertyAreaId", this.propertyAreaDetails);
        this.onGet("/api/v1/:lang/property-type/:propertyTypeId", this.propertyTypeDetails);
    }
    /**
     * method: filter list
     */
    ...
    public async propertyAreaDetails(req: HttpRequest, res: HttpResponse, next: NextFunc) {
        try{
            const lang: string =  req.params.lang;
            const propertyAreaId: string =  req.params.propertyAreaId;
            // filter - act as optional
            const developerId: any =  req.query.developerId;
            const propertyTypeId: any =  req.query.propertyTypeId;
            const completion: any =  req.query.completion;
            ...
            let size: number = parseInt(s, 10);
            if (!size || size < 1) size = 6;
            const propertyArea: IPropertyArea = await this.PropertyAreaProvider.get(propertyAreaId);
            const propertiesByArea: IPropertyPage = await this.PropertyProvider.propertyListByArea(page, size, propertyAreaId, developerId, propertyTypeId, completion, beds);
    ...
    public async propertyTypeDetails(req: HttpRequest, res: HttpResponse, next: NextFunc) {
        try{
            const lang: string =  req.params.lang;
            const propertyTypeId: string =  req.params.propertyTypeId;
            // filter - act as optional
            const developerId: string =  req.params.developerId;
            const propertyAreaId: any =  req.query.propertyAreaId;
            const completion: any =  req.query.completion;
            ...
            let size: number = parseInt(s, 10);
            if (!size || size < 1) size = 6;
            const propertyType: IPropertyType = await this.PropertyTypeProvider.get(propertyTypeId);
            const propertiesByPropertyType: IPropertyPage = await this.PropertyProvider.propertyListByPropertyType(page, size, propertyTypeId, developerId, propertyAreaId, completion, beds);
